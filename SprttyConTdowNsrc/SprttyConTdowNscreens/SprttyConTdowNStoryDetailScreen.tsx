...
import {getStoryById} from '../SprttyConTdowNdata/SprttyConTdowNstoriesData';

type StrsDetailNav = NativeStackNavigationProp<RootParamList, 'StoryDetail'>;

type StrsDetailRoute = RouteProp<RootParamList, 'StoryDetail'>;

const StoryDetailScreen = () => {
  const navigation = useNavigation<StrsDetailNav>();
  const route = useRoute<StrsDetailRoute>();
    ...
    <Layout bounces={false}>
      <View
        ...
        <View style={styles.topBar}>
          ...
          <Pressable onPress={onShare} style={styles.navBtn}>
            <Image source={require('../../assets/i/sportyritcocshr.png')} />
            <Text style={[styles.navBtnText, {color: colors.accentBright}]}>
              Share
            </Text>
          </Pressable>
        </View>
      ...
      <View style={[styles.scrollContent, {paddingBottom: insets.bottom + 24}]}>
        {paragraphs.map((p, i) => (
          <Text
            key={i}
            style={[styles.para, i === 0 ? styles.paraFirst : null]}>
            {p}
          </Text>
        ))}
      </View>

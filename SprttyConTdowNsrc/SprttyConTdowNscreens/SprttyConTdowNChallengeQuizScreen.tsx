import {
  ...
  QUIZ_SESSION_TOTAL,
  QUIZ_TIMER_SEC,
} from '../SprttyConTdowNconstants/SprttyConTdowNquiz';
import {
  useFocusEffect,
  useNavigation,
  useRoute,
...
import {pickQuizBatch} from '../SprttyConTdowNdata/SprttyConTdowNquizData';
import type {QuizOutcome} from '../SprttyConTdowNdata/SprttyConTdowNquizTypes';
...
import {headerPaddingTop} from '../SprttyConTdowNutils/SprttyConTdowNindex';
...
type ChallengeQuizRoute = RouteProp<RootParamList, 'ChallengeQuiz'>;

const ChallengeQuizScreen = () => {
  const navigation = useNavigation<ChallengeQuizNav>();
  const route = useRoute<ChallengeQuizRoute>();
  const restartKey = route.params?.restart ?? 0;
  ...
  const [seconds, setSeconds] = useState(QUIZ_TIMER_SEC);
  ...
  const goResults = useCallback(
    (finalScore: number, finalOutcomes: QuizOutcome[]) => {
      clearAdvance();
  ...
  const advance = useCallback(
    (nextOutcomes: QuizOutcome[], nextScore: number) => {
      const nextIdx = index + 1;
      if (nextIdx >= QUIZ_SESSION_TOTAL) {
        goResults(nextScore, nextOutcomes);
        return;
      }
  ...
  const onTimeout = useCallback(() => {
    setLocked(true);
    const next = [...outcomes, 'timeout' as const];
    advanceTimerRef.current = setTimeout(() => {
      advance(next, score);
    }, 550);
  }, [outcomes, score, advance]);

  useFocusEffect(
    useCallback(() => {
      roundRef.current = restartKey;
      setQuestions(pickQuizBatch(QUIZ_SESSION_TOTAL));
      setIndex(0);
      setScore(0);
      setOutcomes([]);
      setSeconds(QUIZ_TIMER_SEC);
  ...
  const onPick = (optionIdx: number) => {
    ...
    const outcome: QuizOutcome = ok ? 'correct' : 'wrong';
    const nextOutcomes = [...outcomes, outcome];
    const nextScore = ok ? score + 1 : score;
    advanceTimerRef.current = setTimeout(() => {
      advance(nextOutcomes, nextScore);
    }, 650);
  };
  ...
  const progress = index / QUIZ_SESSION_TOTAL;
        ...
        <View style={styles.headerRow}>
          <View>
            <Text style={styles.qMeta}>
              QUESTION {index + 1} / {QUIZ_SESSION_TOTAL}
            </Text>
          ...
          <View style={styles.dots}>
            {Array.from({length: QUIZ_SESSION_TOTAL}, (_, i) => {
              ...
              return (
                <View key={i} style={[styles.dot, {backgroundColor: dotBg}]} />
              );
            })}
          </View>
        </View>
          ...
          {current.options.map((label, i) => {
            ...
            return (
              <Pressable
                ...
                disabled={locked}
                onPress={() => onPick(i)}
                style={[styles.optionRow, {borderColor: btnBorder}]}>
